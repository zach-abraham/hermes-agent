...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_needs_zach_callback(self, query, data: str) -> None:
        ...
        callback_data: nz:a:<id> (approve) or nz:d:<id> (deny/dismiss).
        Resolves needs-zach.jsonl via escalation_phone.py (same ledger as dashboard).
        """
        parts = (data or "").split(":", 2)
        if len(parts) != 3 or parts[0] != "nz" or parts[1] not in ("a", "d"):
            await query.answer(text="Invalid decision button.")
            return
        choice, eid = parts[1], parts[2]
        caller_id = str(getattr(query.from_user, "id", "") or "")
        if caller_id not in {"1704842707"}:
            try:
                ok_auth = self._is_callback_user_authorized(
        ...
        action = "approve" if choice == "a" else "deny"
        import asyncio
        from pathlib import Path as _Path

        script = _Path.home() / "clawd" / "hal-selfimprove" / "tools" / "escalation_phone.py"
        if not script.is_file():
            await query.answer(text="Resolver missing on host.")
            return
        ...
        try:
            ...
            try:
                ...
                payload = {
                    "ok": False,
                    "error": out[:200] or (err_b or b"").decode()[:200],
                }
        except Exception as exc:
            await query.answer(text=("Resolver error: %s" % exc)[:180])
            return
        ...
        if payload.get("ok"):
            ...
            user_display = getattr(query.from_user, "first_name", "Zach")
            nl = chr(10)
            edit = f"{verb} by {user_display}{nl}{eid}{nl}{title}"
            if action == "approve":
                edit += f"{nl}handed to HAL backlog"
            try:
                await query.edit_message_text(text=edit, reply_markup=None)
            except Exception:
                pass
